...
## 13.14.2-tanu-1.54.1
### General
- フルインデックスの進捗を取得できるように
- 初期表示のクエリの範囲を制限

## 13.14.2-tanu-1.54.0
### General
- ユーザーリスト、ユーザーノートは初期表示は遅くなるので10日前までで一旦区切る
- グローバルタイムラインを10日前までに制限
- 使用していない notes APIを削除
- ruby, border, unixtime に対応
- 青空文庫記法でもMFMにルビを付けれる用に修正

## 13.14.2-tanu-1.53.0
### General
- Stripeを利用したサブスクリプション機能の実装
- フォームの上部固定時にチャンネルのリストアをしないように修正
- 絵文字一覧の初期表示でカテゴリの折りたたみ状態をデフォルトに

## 13.14.2-tanu-1.52.0
### General
- fix: 禁止キーワードを含むノートやリモートの通知を引き起こすブロックされた投稿の処理がDelayed Queueに追加されて再処理される問題 (#468)
- 絵文字の変更申請用のユーザー情報取得処理の負荷が高い問題の修正

## 13.14.2-tanu-1.52.0
### General
- フォームに複数行入力した際の縦幅自動拡大に対応
- 絵文字の変更申請に対応

## 13.14.2-tanu-1.51.2
### General
- ローカルユーザーがまだ誰もフォローしていないリモートユーザーによる通知を引き起こす可能性のある投稿を拒否できるように (MisskeyIO#462)
- フォームのドラフトが不正に残ったままになる不具合の修正
...
- リモートサーバーでの表示時に、ピン留めでローカルのみ/フォロワー限定/ダイレクトであったときに公開しないように
- 登録を拒否するメールアドレスのドメインを手動で設定できるように
- ミュートしているユーザーのリアクションを隠すように対応
- 削除して編集時のCWの不具合修正
- チャンネル返信したときにパブリックになるケースがある不具合の修正
- CWに返信したときに注釈部分は引き継がないように仕様変更
- アンテナでローカルの投稿のみ収集できるように
...
- ミュートなどができるようにするため凍結されたユーザーのプロフィールを一般ユーザーが閲覧できるように
- CWの注釈部分に複数行入力できるように
- リプライやDM時に同じ公開範囲しか選べないように修正
- DMはホームおよびソーシャルタイムラインに流れないように修正

### Server
- ホワイトリストモード有効化時に自サーバーの投稿が引用されると引用部が欠落する問題の修正

### Client
- 0dXでダイスロールがエラーになる不具合の修正
- ユーザーのプロフィール画面にノートタブを再追加
...
## 13.14.2-tanu-1.44.5
### General
- イベント、サークルのアーカイブをできるように修正
- iOSでバックグラウンド再生中の音楽がMisskeyの通知音などを受けて停止してしまう現象の修正
- ページ、プロフィールで絵文字の補完ができるように対応

## 13.14.2-tanu-1.44.4
### General
- サークル主がイベントに紐づくサークル情報を編集できない不具合の修正

## 13.14.2-tanu-1.44.3
### General
- サークルのバナーが更新できない不具合の修正
- EventCircleのcircleIdが更新できない不具合の修正

## 13.14.2-tanu-1.44.2
### General
- サークルのアップデートではなく新規作成になってしまう不具合の修正

## 13.14.2-tanu-1.44.1
### General
- イベントのサークルのバナーが設定できない不具合の修正
...
## 13.14.2-tanu-1.43.4
### General
- アンテナに検索不可チャンネルのノートが引っかからないように修正
- リストに検索不可チャンネルのノートが引っかからないように修正
- x.comでも展開ができるように
- イベントにカレンダー機能を暫定追加
...
- fix: #154 リモートユーザーにプロフィールメニューからDMを送ろうとするとhostがない
- TanukeyサポートDiscordを更新

## 13.14.2-tanu-1.43.2
### General
- 固定チャンネルの設定上限を50個に緩和
- コードブロックを横スクロールするとスワイプされる不具合の修正
- fix: 絵文字管理画面の表示不具合の修正

## 13.14.2-tanu-1.43.1
### General
- イベント、サークルのstyleを調整
- fix: チャンネル切り替えがエラーになっている不具合の修正
...
## 13.14.2-tanu-1.38.0
### General
- ページのクリップをノートごとのフッターに表示するオプションをデフォルトONに
- fix: iPhoneで横回転すると文字が大きくなったままになる不具合の修正
...
## 13.14.2-tanu-1.36.0
### General
- fix: X(Twitter)のプレビューが開けない不具合の修正
- fix: 自リアクションのドロップシャドウを削除
- 固定チャンネルのピンどめの折りたたみを記憶するように実装

## 13.14.2-tanu-1.35.0
### General
- VC機能の削除
- タイムラインの仕様を変更(ローカル,ソーシャル,管理者固定→フィードタブに移動、ユーザー固定チャンネル→チャンネルタブに移動)

## 13.14.2-tanu-1.34.0
### General
- ポップアップ表示時にヘッダーの左側に余白が入らない不具合の修正
- about.emojisで横長の絵文字の内容を読めるように
- ノート中の絵文字のサイズを2emにrevert
- 投稿用ダイアログにフォントサイズの指定が効くように
- OAuth 2.0のサポート
- お知らせ機能の強化
	- ユーザー個別のお知らせを作成可能に
...
## 13.13.2-tanu-1.30.0
### General
- fix: プライベートチャンネルにする前からフォローしているユーザに投稿が見える不具合修正
- feat: チャンネルのオーナー以外にモデレーターを追加できるように
...
### General
- Misskey13.13.2をマージ
- movファイルの再生がSafari以外でできない不具合の修正
...
## 13.13.1-tanu-1.29.0
### General
- feat: ドロップダウンリストでタブの切り替えができるように修正
- fix: AAの投稿時に本文がないとundefinedの文字が入る不具合の修正
...
- fix: onClose時にチャンネルの再接続を試みるように修正
- fix: textが空のとき、引用ができない不具合の修正
- fix: publicのノート検索が正しくできない不具合の修正

## 13.13.0-beta.7-tanu-1.26.1
### General
- fix: フォームからチャンネルの切り替えが行われたらvisibilityをpublicに戻す

## 13.13.0-beta.7-tanu-1.26.0
### General
- fix: 画像の添付編集とアンケートの編集ができなくなっている不具合の修正
- feat: 投稿時のチャンネルの切り替え機能の実装
- feat: 複数チャンネルの取得処理を一括で行えるように修正

## 13.13.0-beta.7-tanu-1.25.5
### General
- 13.13.0-beta.7をマージ
- style: 固定表示時にフォームからチャンネルが非表示にされる仕様をオミット
- style: ノートのプレビューをメニューの下に表示するように修正
- style: フォームのチャンネル表示にツールチップを表示するように修正
- fix: unicode絵文字のキーワードが削除された更新のrevert
- fix: AAにダイスロールが適用されていなかった不具合の修正

## 13.13.0-beta.3-tanu-1.25.1
### General
- fix: 外部の絵文字取得に失敗する不具合の修正

## 13.13.0-beta.3-tanu-1.25.0
### General
- feat: プライベートチャンネル、ボイスチャンネルの作成にロールのポリシーが必要に変更

## 13.13.0-beta.3-tanu-1.24.0
### General
- feat: プライベートチャンネルの実装
- feat: タブを左端でスワイプしたら右端に移動するように修正（逆も）
- style: アスキーアート切り替えアイコンをAAに変更

## 13.13.0-beta.3-tanu-1.23.0
### General
- feat: アスキーアートにダークモードを適用するかのオプションを追加
- fix: ボイスチャンネルではスワイプ切り替えを無効化
- feat: ボイスチャットチャンネルの現在の参加者をわかるように

## 13.13.0-beta.3-tanu-1.22.0
### General
- enhance: voicechatのuser情報をキャッシュするように変更
- feat: VCをチャンネルごとに有効化対応
- feat: VCで話している人を光らせる用に対応
- feat: ボイスチャンネルタブの追加
- fix: タブの読み込み後に上に少しずれる現象の暫定対応
- style: aaの表示は白背景に黒文字に固定

## 13.13.0-beta.3-tanu-1.21.0
### General
- feat: 固定チャンネルにノートのピンどめ表示機能を追加

## 13.13.0-beta.3-tanu-1.20.0
### General
- feat: ボイスチャット機能のプロト実装の追加

## 13.13.0-beta.3-tanu-1.19.3
### General
fix: AAの横スクロールを行っているときにはスワイプを無効化するように修正
fix: AAの数値文字参照が10進数扱いになってた不具合の修正
feat: フォームでアスキーアートを使用するかを記憶するように

## 13.13.0-beta.3-tanu-1.19.2
### General
- fix: 畳む表示の場合もある程度コンテンツを見せるように修正

## 13.13.0-beta.3-tanu-1.19.0
### General
- Misskey固有の実績の削除
- feat: feat: チャンネル内のノートを畳まないチャンネルオプションの追加

### Client
- feat: モバイル環境でのスワイプでのチャンネル切り替えに対応
- feat: ノートを畳む基準を文字数は見ずに30以上に緩和

## 13.13.0-beta.3-tanu-1.18.0
### General
- feat: アスキーアートの入力フォームに対応
- style: ダイス結果にbold指定を追加
- feat: 最大文字数を8192文字に修正

## 13.13.0-beta.3-tanu-1.17.0
### General
- feature: ダイスロールに対応
- feature: AAの数値参照文字列の変換に対応
- style: モーダルの投稿フォームの横幅をPCのノートの最大値に合わせて800pxに修正

## 13.13.0-beta.3-tanu-1.16.7
### General
...
## 13.13.0-beta.3-tanu-1.16.1
### General
- style: スクリーンサイズでのアスキーアートのサイズの修正
...
- fix: ユーザー言語が切り替えられない、Androidで初期言語が英語になる不具合の修正

## 13.13.0-beta.3-tanu-1.15.0
### Client
- 新着絵文字と申請中の絵文字をグループで表示するように修正

## 13.13.0-beta.3-tanu-1.14.7
### Client
- fix: ノート検索でチャンネル以外が引っかからなくなっていた不具合の修正

## 13.13.0-beta.3-tanu-1.14.6
### Client
- fix: タブ切り替えでチャンネル切り替えが正しくできてなかった不具合の修正

## 13.13.0-beta.3-tanu-1.14.5
### Client
- style: MkStickyContainerを重ねた場合に上に隙間が開かないように修正
- fix: tabの初期値がない場合は'public'を設定するように修正
...
### Client
- fix: boot時に言語設定を取得できない不具合の修正

## 13.13.0-beta.3-tanu-1.14.1
### Client
- fix: channel作成時のオプション設定が保存されてない不具合の修正

## 13.13.0-beta.3-tanu-1.14.0
### General
...
- feature: 連合なしのNoteは非ログインユーザーに表示させないように修正
...
### Client
- fix: デッキUIでパブリックに投稿できない不具合の修正

## 13.13.0-beta.3-tanu-1.10.4
### Client
- fix: チャンネル切り替えの挙動が正しくない不具合の修正
- fix: ドラフトの絵文字は該当するキーを入力されても表示しないように修正
- fix: 絵文字を追加した後に画像のリンクがエラーになる不具合の修正
- fix: emojiのドラフトが黄色表示になってない不具合の修正
- style: ドラフト表示の表記を修正

## 13.13.0-beta.3-tanu-1.10.3
### Client
- fix: ページの編集ができない不具合の修正

## 13.13.0-beta.3-tanu-1.10.2
### Client
- fix: ページが表示できない不具合の修正
- fix: 開発環境でアバター画像が表示できない不具合の修正

## 13.13.0-beta.1-tanu-1.10.1
### Client
- fix: TLの上部にフォームを表示するとエラーになる不具合の修正

## 13.13.0-beta.1-tanu-1.10.0
### General
- LTL固定チャンネルにユーザー設定での追加をサポート
- リリースノートを更新してない場合に404にならないようにリンクを修正
- fix: LTLの新規投稿が反映されない不具合の修正

## 13.13.0-beta.1-tanu-1.9.1
### General
- fix: メタ情報の固定チャンネル情報を一般ユーザーが取得できなかった不具合の修正

## 13.13.0-beta.1-tanu-1.9.0

### General
- ユーザープロフィールのノートページを削除しコンテンツをプロフィール下部に移動
- LTLの固定チャンネル機能の実装

## 13.13.0-beta.1-tanu-1.8.0 (Misskey merge)
...
### General
- チャンネル、デッキUIで正しく読み込まれない不具合の修正

## 13.12.2-tanu-1.6.0

### General
- 1度見たRenoteの再リノートは畳んで表示されるように修正

## 13.12.2-tanu-1.5.0

### General
- ユーザー検索で@name@hostの形式で検索しても照会するようにする
- 管理者の絵文字登録での画像の入れ替えに対応
...
- 定期的に表示されるDonationのポップアップ機能を削除
- Postしたらノートの投稿フォームのハッシュタグ枠を初期化するように修正
- ハッシュタグのサジェストにローカルで使われたタグのみ表示するように修正
...
### Client
- チャンネルを開いた状態でノートするとチャンネルに投稿するように変更がスマホで適用されてなかった不具合の修正

## 13.12.2-tanu-1.3.0

### Client
- チャンネルとページをナビゲーションバーのデフォルト値に追加
- ナビゲーションバーにライトモード、ダークモードの切り替えを追加
- チャンネルを開いた状態でノートするとチャンネルに投稿するように変更
...
- 猫モードでノートの文が変わらないように修正
- ノートのプレビューでリンクのプレビューも表示するように修正
- リモートユーザーを通報した場合に通報にリモートユーザーの情報が載るように修正
- 広告の強制表示オプションの追加
- ノートの先頭空白が除去される機能のオミット
- 繋がりが非公開の場合は"非公開"と表示するように変更
- ページの検索機能の追加
