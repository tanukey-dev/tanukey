<template>
	<MkA v-adaptive-bg :to="forModeration ? `/secure/admin/roles/${role.id}` : `/secure/roles/${role.id}`"
		class="_panel" :class="$style.root" tabindex="-1" :style="{ '--color': role.color }">
		<div :class="$style.title">
			...
			<template v-if="detailed">
				...
				<span v-else-if="role.target === 'conditional'" :class="$style.users">({{ i18n.ts._role.conditional
					}})</span>
			</template>
		</div>
