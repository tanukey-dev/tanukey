...
export default function (
	...
	const ast = (props.plain ? mfm.parseSimple : mfm.parse)(
		convertAozoraRuby(props.text),
	);
						...
						switch (token.props.name) {
							case "tada": {
								...
								style =
									"font-size: 150%;" +
									(useAnim
										? `animation: tada ${speed} linear infinite both; animation-delay: ${delay};`
										: "");
								break;
							}
							case "jelly": {
						...
						return [
							h(MkAsciiArt, {
								text: token.props.text
									.replace(/(\r\n|\n|\r)/g, "\n")
									.replace(/&#x([a-zA-Z0-9]{4})[;]{0,1}/g, (match, c1) =>
										String.fromCharCode(parseInt(c1, 16)),
									),
							}),
						];
					}
							...
							if (
								...
								return [
									h(MkCustomEmoji, {
										...
										// eslint-disable-next-line @typescript-eslint/no-unnecessary-condition
										url: props.emojiUrls
											? props.emojiUrls[token.props.name]
											: null,
										normal: props.plain,
