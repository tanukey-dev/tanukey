...
<div class="_gaps">
	...
	<MkInput v-model="role.loginBonusAdditionalPoint" type="number">
		<template #label>{{ i18n.ts._role.loginBonusAdditionalPoint }}</template>
		<template #caption>{{ i18n.ts._role.descriptionOfLoginBonusAdditionalPoint }}</template>
	</MkInput>
	...
	<FormSlot>
		...
		<div class="_gaps_s">
			...
			<MkFolder v-if="matchQuery([i18n.ts._role._options.canCreateToken, 'canCreateToken'])">
				<template #label>{{ i18n.ts._role._options.canCreateToken }}</template>
				<template #suffix>
					<span v-if="role.policies.canCreateToken.useDefault" :class="$style.useDefaultLabel">{{ i18n.ts._role.useBaseValue }}</span>
					<span v-else>{{ role.policies.canCreateToken.value ? i18n.ts.yes : i18n.ts.no }}</span>
					<span :class="$style.priorityIndicator"><i :class="getPriorityIcon(role.policies.canCreateToken)"></i></span>
				</template>
				<div class="_gaps">
					<MkSwitch v-model="role.policies.canCreateToken.useDefault" :readonly="readonly">
						<template #label>{{ i18n.ts._role.useBaseValue }}</template>
					</MkSwitch>
					<MkSwitch v-model="role.policies.canCreateToken.value" :disabled="role.policies.canCreateToken.useDefault" :readonly="readonly">
						<template #label>{{ i18n.ts.enable }}</template>
					</MkSwitch>
					<MkRange v-model="role.policies.canCreateToken.priority" :min="0" :max="2" :step="1" easing :textConverter="(v) => v === 0 ? i18n.ts._role._priority.low : v === 1 ? i18n.ts._role._priority.middle : v === 2 ? i18n.ts._role._priority.high : ''">
						<template #label>{{ i18n.ts._role.priority }}</template>
					</MkRange>
				</div>
			</MkFolder>
			...
			<MkFolder v-if="matchQuery([i18n.ts._role._options.additionalDriveCapacity, 'additionalDriveCapacityMb'])">
				...
				<template #suffix>
					<span v-if="role.policies.additionalDriveCapacityMb.useDefault" :class="$style.useDefaultLabel">{{ i18n.ts._role.useBaseValue }}</span>
					<span v-else>{{ role.policies.additionalDriveCapacityMb.value + 'MB' }}</span>
					<span :class="$style.priorityIndicator"><i :class="getPriorityIcon(role.policies.additionalDriveCapacityMb)"></i></span>
				</template>
				<div class="_gaps">
					<MkSwitch v-model="role.policies.additionalDriveCapacityMb.useDefault" :readonly="readonly">
						<template #label>{{ i18n.ts._role.useBaseValue }}</template>
					</MkSwitch>
					<MkInput v-model="role.policies.additionalDriveCapacityMb.value" :disabled="role.policies.additionalDriveCapacityMb.useDefault" type="number" :readonly="readonly">
						<template #suffix>MB</template>
					</MkInput>
				</div>
			</MkFolder>
