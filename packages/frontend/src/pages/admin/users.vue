				...
				<div class="_gaps">
					<div :class="$style.inputs">
						<MkSelect v-model="sort" style="flex: 1;">
							...
							<option value="-createdAt">{{ i18n.ts.registeredDate }} ({{ i18n.ts.ascendingOrder }})
							</option>
							<option value="+createdAt">{{ i18n.ts.registeredDate }} ({{ i18n.ts.descendingOrder }})
							</option>
					...
					<div :class="$style.inputs">
						...
						<MkInput v-model="searchHost" style="flex: 1;" type="text" :spellcheck="false"
							:disabled="pagination.params.origin === 'local'" @update:modelValue="$refs.users.reload()">
							<template #prefix>@</template>
					...
					<MkPagination v-slot="{ items }" ref="paginationComponent" :pagination="pagination">
						<div :class="$style.users">
							<MkA v-for="user in items" :key="user.id"
								v-tooltip.mfm="`Last posted: ${dateString(user.updatedAt)}`" :class="$style.user"
