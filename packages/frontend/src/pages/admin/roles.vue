		...
		<MkStickyContainer>
			...
			<MkSpacer :contentMax="700">
				<div class="_gaps">
					<MkFolder>
						...
						<div class="_gaps_s">
							...
							<MkFolder v-if="matchQuery([i18n.ts._role._options.rateLimitFactor, 'rateLimitFactor'])">
								...
								<MkRange :modelValue="policies.rateLimitFactor * 100" :min="30" :max="300" :step="10"
									:textConverter="(v) => `${v}%`"
									@update:modelValue="v => policies.rateLimitFactor = (v / 100)">
									<template #caption>{{ i18n.ts._role._options.descriptionOfRateLimitFactor
										}}</template>
								</MkRange>
							</MkFolder>
							...
							<MkFolder
								v-if="matchQuery([i18n.ts._role._options.inviteExpirationTime, 'inviteExpirationTime'])">
							...
							<MkFolder
								v-if="matchQuery([i18n.ts._role._options.canManageCustomEmojis, 'canManageCustomEmojis'])">
								<template #label>{{ i18n.ts._role._options.canManageCustomEmojis }}</template>
								<template #suffix>{{ policies.canManageCustomEmojis ? i18n.ts.yes : i18n.ts.no
									}}</template>
							...
							<MkFolder
								v-if="matchQuery([i18n.ts._role._options.canRequestCustomEmojis, 'canRequestCustomEmojis'])">
								<template #label>{{ i18n.ts._role._options.canRequestCustomEmojis }}</template>
								<template #suffix>{{ policies.canRequestCustomEmojis ? i18n.ts.yes : i18n.ts.no
									}}</template>
							...
							<MkFolder v-if="matchQuery([i18n.ts._role._options.canCreateToken, 'canCreateToken'])">
								...
								<template #suffix>{{ policies.canCreateToken ? i18n.ts.yes : i18n.ts.no }}</template>
								<MkSwitch v-model="policies.canCreateToken">
									<template #label>{{ i18n.ts.enable }}</template>
								</MkSwitch>
							</MkFolder>
							...
							<MkFolder
								v-if="matchQuery([i18n.ts._role._options.additionalDriveCapacity, 'additionalDriveCapacityMb'])">
							...
							<MkFolder
								v-if="matchQuery([i18n.ts._role._options.noteEachClipsMax, 'noteEachClipsLimit'])">
							...
							<MkFolder
								v-if="matchQuery([i18n.ts._role._options.userEachUserListsMax, 'userEachUserListsLimit'])">
							...
							<MkFolder
								v-if="matchQuery([i18n.ts._role._options.canCreatePrivateChannel, 'canCreatePrivateChannel'])">
								<template #label>{{ i18n.ts._role._options.canCreatePrivateChannel }}</template>
								<template #suffix>{{ policies.canCreatePrivateChannel ? i18n.ts.yes : i18n.ts.no
									}}</template>
					...
					<MkButton primary rounded @click="create"><i class="ti ti-plus"></i> {{ i18n.ts._role.new }}
					</MkButton>
					<div class="_gaps_s">
						<MkFoldableSection>
							<template #header>Manual roles</template>
							<div class="_gaps_s">
								<MkRolePreview v-for="role in roles.filter(x => x.target === 'manual')" :key="role.id"
									:role="role" :forModeration="true" />
							</div>
						</MkFoldableSection>
						<MkFoldableSection>
							<template #header>Conditional roles</template>
							<div class="_gaps_s">
								<MkRolePreview v-for="role in roles.filter(x => x.target === 'conditional')"
									:key="role.id" :role="role" :forModeration="true" />
							</div>
						</MkFoldableSection>
					</div>
				</div>
...
function create() {
	router.push("/secure/admin/roles/new");
}
