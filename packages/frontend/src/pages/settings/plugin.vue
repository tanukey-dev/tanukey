<template>
	<div class="_gaps_m">
		<FormLink to="/secure/settings/plugin/install"><template #icon><i class="ti ti-download"></i></template>{{
		...
		<FormSection>
			...
			<div class="_gaps_s">
				<div v-for="plugin in plugins" :key="plugin.id" class="_panel _gaps_s" style="padding: 20px;">
					<span style="display: flex;"><b>{{ plugin.name }}</b><span style="margin-left: auto;">v{{
						plugin.version
							}}</span></span>

					<MkSwitch :modelValue="plugin.active" @update:modelValue="changeActive(plugin, $event)">{{
					...
					<div class="_buttons">
						<MkButton v-if="plugin.config" inline @click="config(plugin)"><i class="ti ti-settings"></i> {{
							i18n.ts.settings }}</MkButton>
						<MkButton inline danger @click="uninstall(plugin)"><i class="ti ti-trash"></i> {{
							i18n.ts.uninstall }}
						</MkButton>
					</div>
				</div>
