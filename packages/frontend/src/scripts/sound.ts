...
for (const target of Object.keys(soundConfigStore.state) as Array<
	keyof typeof soundConfigStore.state
>) {
	const value = localStorage.getItem(`miux:${target}`);
	if (value) {
		soundConfigStore.set(
			target,
			JSON.parse(
				value,
			) as (typeof soundConfigStore.def)[typeof target]["default"],
		);
...
export function play(
	type: "noteMy" | "note" | "antenna" | "channel" | "notification",
) {
