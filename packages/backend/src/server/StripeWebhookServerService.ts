	...
	public createServer(
		...
		fastify.post(
			...
			async (request, reply) => {
				...
				const preprocessEvent = async (
					eventData: any,
				): Promise<{ userProfile: UserProfile; subscription: any }> => {
				...
				const { userProfile, subscription } = await preprocessEvent(
					event.data.object,
				);
				...
				switch (event.type) {
					case "customer.subscription.created": {
						// サブスクリプションが新規に作成された場合
						const subscriptionPlan =
							await this.subscriptionPlansRepository.findOneByOrFail({
						...
						if (subscription.status === "active") {
							await this.roleService
								...
								.then(async (roles) => {
									...
									if (
										!roles.some((role) => role.id === subscriptionPlan.roleId)
									) {
										await this.roleService.assign(
											user.id,
											subscriptionPlan.roleId,
										);
									}
								});
						}
						...
						const status = await this.subscriptionStatusesRepository
							.createQueryBuilder("status")
							.andWhere("status.userId = :userId", { userId: user.id })
							.andWhere("status.planId = :planId", {
								planId: subscriptionPlan.id,
							})
							.getOne();
						if (status) {
							await this.subscriptionStatusesRepository.update(
								{ id: status.id },
								{
					...
					case "customer.subscription.deleted": {
						...
						await this.roleService
							.getUserRoles(userProfile.userId)
							.then(async (roles) => {
