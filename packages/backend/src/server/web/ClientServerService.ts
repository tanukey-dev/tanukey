...
export class ClientServerService {
	...
	public createServer(
		...
		// User
		const getAtUser = async (request: any, reply: any) => {
		...
		const getUsersUser = async (request: any, reply: any) => {
			const user = await this.usersRepository.findOneBy({
		...
		fastify.get<{ Params: { user: string } }>("/users/:user", getUsersUser);
		...
		// Note
		const getNotesNote = async (request: any, reply: any) => {
		...
		fastify.get<{ Params: { note: string } }>("/notes/:note", getNotesNote);
		...
		// Page
		const getPagesPage = async (request: any, reply: any) => {
			const { username, host } = Acct.parse(request.params.user);
			const user = await this.usersRepository.findOneBy({
				usernameLower: username.toLowerCase(),
				host: host ?? IsNull(),
		...
		fastify.get<{ Params: { user: string; page: string } }>(
			"/secure/@:user/pages/:page",
			getPagesPage,
		);

		// Channel
		const getChannel = async (request: any, reply: any) => {
