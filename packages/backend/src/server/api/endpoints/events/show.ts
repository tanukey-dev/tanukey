...
import type { Event, EventsRepository } from "@/models/Repositories.js";
		...
		super(meta, paramDef, async (ps, me) => {
			if (ps.time) {
				...
				return await Promise.all(
					events.map((x) => this.eventEntityService.pack(x, me)),
				);
			} else if (ps.eventIds) {
				const events = await this.eventsRepository
					.createQueryBuilder("event")
				...
				return await Promise.all(
					_events.map((x) => this.eventEntityService.pack(x, me)),
				);
			} else {
