...
export default class extends Endpoint<typeof meta, typeof paramDef> {
	...
	constructor(
		...
		super(meta, paramDef, async (ps, me) => {
			...
			const plan = await this.subscriptionPlansRepository.findOneBy({
				id: ps.planId,
			});
			...
			let userProfile = await this.userProfilesRepository.findOneBy({
				userId: me.id,
			});
			if (!user || !userProfile) {
			...
			if (!userProfile.stripeCustomerId) {
				...
				userProfile = await this.userProfilesRepository.findOneByOrFail({
					userId: user.id,
				});
			}

			let subscription = await subscriptionStatusesRepository
			...
			if (user.subscriptionPlanId) {
				if (subscription === null) {
					...
					subscription = await subscriptionStatusesRepository
						.createQueryBuilder("status")
						.andWhere("status.userId = :userId", { userId: user.id })
						.andWhere("status.planId = :planId", { planId: plan.id })
						.getOne();
