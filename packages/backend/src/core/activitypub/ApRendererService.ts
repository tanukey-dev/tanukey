...
export class ApRendererService {
	...
	public async renderFollowUser(id: User["id"]) {
		const user = (await this.usersRepository.findOneByOrFail({ id: id })) as
			| PartialLocalUser
			| PartialRemoteUser;
	...
	public async renderLike(
		...
		if (reaction.startsWith(":")) {
			...
			const emoji = (
				await this.customEmojiService.localEmojisCache.fetch()
			).get(name);
	...
	public async renderNote(note: Note, dive = true): Promise<IPost> {
		...
		if (note.replyId) {
			...
			if (inReplyToNote != null) {
				const inReplyToUser = await this.usersRepository.findOneBy({
		...
		if (profile.fields) {
			for (const field of profile.fields) {
				attachment.push({
					...
					name: field.name,
					value:
						field.value != null && field.value.match(/^https?:/)
	...
	public renderUndo(object: any, user: { id: User["id"] }): IUndo {
		const id =
			typeof object.id === "string" && object.id.startsWith(this.config.url)
				? `${object.id}/undo`
				: undefined;
