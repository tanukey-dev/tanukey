...
export class ApNoteService {
	...
	public validateNote(object: IObject, uri: string): Error | null {
		...
		if (
			...
			return new Error(
				`invalid Note: id has different host. expected: ${expectHost}, actual: ${this.utilityService.extractDbHost(object.id)}`,
			);
		}
	...
	public async createNote(
		...
		const actor = (await this.apPersonService.resolvePerson(
		...
		const files = await Promise.all(
			toArray(note.attachment).map((attach) =>
				limit(() =>
					this.apImageService.resolveImage(actor, {
						...attach,
	...
	public async extractEmojis(
		...
		return await Promise.all(
			eomjiTags.map(async (tag) => {
				...
				return await this.emojisRepository
					.insert({
