...
export class RemoteUserResolveService {
	...
	public async resolveUser(
		...
		if (host == null) {
			...
			return (await this.usersRepository
				.findOneBy({ usernameLower, host: IsNull() })
				.then((u) => {
					if (u == null) {
	...
	private async resolveSelf(acctLower: string) {
		...
		const finger = await this.webfingerService
			...
			.catch((err) => {
				this.logger.error(
					`Failed to WebFinger for ${chalk.yellow(acctLower)}: ${err.statusCode ?? err.message}`,
				);
				throw new Error(
					`Failed to WebFinger for ${acctLower}: ${err.statusCode ?? err.message}`,
				);
			});
