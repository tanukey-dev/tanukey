...
<html>
	...
	public async validateEmailForAccount(emailAddress: string): Promise<{
		...
		if (meta.enableActiveEmailValidation) {
			...
			if (
				validated.valid &&
				meta.enableVerifymailApi &&
				meta.verifymailAuthKey != null
			) {
	...
	private async verifyMail(emailAddress: string): Promise<{
		...
		const meta = await this.metaService.fetch();
		if (meta.verifymailAuthKey == null)
			throw new Error("verifymailAuthKey is not set");

		const endpoint =
			"https://verifymail.io/api/" +
			emailAddress +
			"?key=" +
			meta.verifymailAuthKey;
