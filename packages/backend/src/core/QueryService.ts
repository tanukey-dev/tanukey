...
export class QueryService {
	...
	@bindThis
	public generateBlockedUserQuery(
	...
	@bindThis
	public generateBlockQueryForUsers(
		q: SelectQueryBuilder<any>,
		me: { id: User["id"] },
	): void {
		const blockingQuery = this.blockingsRepository
		...
		const blockedQuery = this.blockingsRepository
			.createQueryBuilder("blocking")
			.select("blocking.blockerId")
			.where("blocking.blockeeId = :blockeeId", { blockeeId: me.id });
	...
	public generateChannelQuery(
		...
		if (me == null) {
			...
			const channelFollowingQuery = this.channelFollowingsRepository
				.createQueryBuilder("channelFollowing")
						...
						.orWhere(
							new Brackets((qb2) => {
								qb2
									...
									.where(
										`note.channelId IN (${channelFollowingQuery.getQuery()})`,
									)
									.andWhere(
										new Brackets((qb3) => {
											qb3
														...
														qb2.where("channel.isPrivate = TRUE").andWhere(
															new Brackets((qb3) => {
																qb3
																	...
																	.orWhere(
																		":id = ANY(channel.moderatorUserIds)",
																		{ id: me.id },
																	)
																	.orWhere("channel.userId = :id", {
																		id: me.id,
																	});
															}),
														);
	...
	@bindThis
	public generateMutedNoteQuery(
		q: SelectQueryBuilder<any>,
		me: { id: User["id"] },
	): void {
		const mutedQuery = this.mutedNotesRepository
	...
	@bindThis
	public generateMutedNoteThreadQuery(
		q: SelectQueryBuilder<any>,
		me: { id: User["id"] },
	): void {
		const mutedQuery = this.noteThreadMutingsRepository
	...
	@bindThis
	public generateMutedUserQueryForUsers(
		q: SelectQueryBuilder<any>,
		me: { id: User["id"] },
	): void {
		const mutingQuery = this.mutingsRepository
	...
	@bindThis
	public generateVisibilityQuery(
		q: SelectQueryBuilder<any>,
		me?: { id: User["id"] } | null,
	): void {
	...
	@bindThis
	public generateMutedUserRenotesQueryForNotes(
		q: SelectQueryBuilder<any>,
		me: { id: User["id"] },
	): void {
