...
export class NotificationEntityService implements OnModuleInit {
	...
	public async pack(
		...
		const token = notification.appAccessTokenId
			? await this.accessTokensRepository.findOneByOrFail({
					id: notification.appAccessTokenId,
				})
			: null;
		const noteIfNeed =
			NOTE_REQUIRED_NOTIFICATION_TYPES.has(notification.type) &&
			notification.noteId != null
				? hint?.packedNotes != null
					? hint.packedNotes.get(notification.noteId)
					: this.noteEntityService.pack(
							notification.noteId!,
							{ id: meId },
							{
								detail: true,
	...
	@bindThis
	public async packMany(notifications: Notification[], meId: User["id"]) {
