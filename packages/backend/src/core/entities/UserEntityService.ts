...
export class UserEntityService implements OnModuleInit {
	...
	public async getRelation(me: User["id"], target: User["id"]) {
		return awaitAll({
			id: target,
			isFollowing: this.followingsRepository
				.count({
					where: {
						followerId: me,
						followeeId: target,
				...
				.then((n) => n > 0),
			isFollowed: this.followingsRepository
				.count({
					where: {
						followerId: target,
						followeeId: me,
				...
				.then((n) => n > 0),
			hasPendingFollowRequestFromYou: this.followRequestsRepository
				.count({
					where: {
						followerId: me,
						followeeId: target,
				...
				.then((n) => n > 0),
			hasPendingFollowRequestToYou: this.followRequestsRepository
				.count({
					where: {
						followerId: target,
						followeeId: me,
				...
				.then((n) => n > 0),
			isBlocking: this.blockingsRepository
				.count({
					where: {
						blockerId: me,
						blockeeId: target,
				...
				.then((n) => n > 0),
			isBlocked: this.blockingsRepository
				.count({
					where: {
						blockerId: target,
						blockeeId: me,
				...
				.then((n) => n > 0),
			isMuted: this.mutingsRepository
				.count({
					where: {
						muterId: me,
						muteeId: target,
				...
				.then((n) => n > 0),
			isRenoteMuted: this.renoteMutingsRepository
				.count({
					where: {
						muterId: me,
						muteeId: target,
	...
	>(
		...
		const user =
			typeof src === "object"
				? src
				: await this.usersRepository.findOneByOrFail({ id: src });
		...
		const iAmModerator = me
			? await this.roleService.isModerator(me as User)
			: false;
		...
		const pins = opts.detail
			? await this.userNotePiningsRepository
		...
		const isModerator =
			isMe && opts.detail ? this.roleService.isModerator(user) : null;
		const isAdmin =
			isMe && opts.detail ? this.roleService.isAdministrator(user) : null;
		const unreadAnnouncements =
			isMe && opts.detail
				? await this.announcementService.getUnreadAnnouncements(user)
				: null;
		...
		const packed = {
			...
			...(opts.detail
				? {
						...
						isSilenced: this.roleService
							.getUserPolicies(user.id)
							.then((r) => !r.canPublicNote),
						...
						securityKeys: profile!.twoFactorEnabled
							...
							: false,
						roles: this.roleService.getUserRoles(user.id).then((roles) =>
							roles
			...
			...(opts.detail && isMe
				? {
						...
						hasUnreadSpecifiedNotes: this.noteUnreadsRepository
							.count({
								where: { userId: user.id, isSpecified: true },
								take: 1,
							})
							.then((count) => count > 0),
						hasUnreadMentions: this.noteUnreadsRepository
							.count({
