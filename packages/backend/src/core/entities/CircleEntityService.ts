...
export class CircleEntityService {
	...
	public async pack(
		...
		const circle =
			typeof src === "object"
				? src
				: await this.circlesRepository.findOneByOrFail({ id: src });
		const meId = me ? me.id : null;

		const profile = circle.profileImageId
			? await this.driveFilesRepository.findOneBy({ id: circle.profileImageId })
			: null;
		...
		return {
			...
			profileImageUrl: profile
				? this.driveFileEntityService.getPublicUrl(profile)
				: null,
