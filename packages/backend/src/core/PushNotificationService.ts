...
export class PushNotificationService implements OnApplicationShutdown {
	...
	constructor(
		...
		this.subscriptionsCache = new RedisKVCache<SwSubscription[]>(
			this.redisClient,
			"userSwSubscriptions",
			{
				...
				fetcher: (key) =>
					this.swSubscriptionsRepository.findBy({ userId: key }),
	...
	public async pushNotification<T extends keyof PushNotificationsTypes>(
		...
		push.setVapidDetails(this.config.url, meta.swPublicKey, meta.swPrivateKey);
