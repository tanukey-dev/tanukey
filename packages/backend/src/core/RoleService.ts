...
export class RoleService implements OnApplicationShutdown {
	...
	constructor(
		...
		this.roleAssignmentByUserIdCache = new MemoryKVCache<RoleAssignment[]>(
			1000 * 60 * 60 * 1,
		);
	...
	public async getUserRoles(userId: User["id"]) {
		...
		const matchedCondRoles = roles.filter(
			(r) => r.target === "conditional" && this.evalCond(user!, r.condFormula),
		);
	...
	public async getUserBadgeRoles(userId: User["id"]) {
		...
		const assignedBadgeRoles = roles.filter(
			(r) => r.asBadge && assignedRoleIds.includes(r.id),
		);
		...
		if (badgeCondRoles.length > 0) {
			...
			const matchedBadgeCondRoles = badgeCondRoles.filter((r) =>
				this.evalCond(user!, r.condFormula),
			);
	...
	public async getUserPolicies(
		...
		return {
			...
			canRequestCustomEmojis: calc("canRequestCustomEmojis", (vs) =>
				vs.some((v) => v === true),
			),
			canCreateToken: calc("canCreateToken", (vs) =>
				vs.some((v) => v === true),
			),
			canSearchNotes: calc("canSearchNotes", (vs) =>
				vs.some((v) => v === true),
			),
			canCreatePrivateChannel: calc("canCreatePrivateChannel", (vs) =>
				vs.some((v) => v === true),
			),
			canHideAds: calc("canHideAds", (vs) => vs.some((v) => v === true)),
			driveCapacityMb: calc("driveCapacityMb", (vs) => Math.max(...vs)),
			additionalDriveCapacityMb: calc("additionalDriveCapacityMb", (vs) =>
				vs.reduce((sum, el) => sum + el, 0),
			),
	...
	public async assign(
		...
		const created = await this.roleAssignmentsRepository
			.insert({
			...
			.then((x) =>
				this.roleAssignmentsRepository.findOneByOrFail(x.identifiers[0]),
			);
