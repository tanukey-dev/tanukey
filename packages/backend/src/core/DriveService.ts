...
import { FILE_TYPE_BROWSERSAFE, FILE_TYPE_IMAGES } from "@/const.js";
...
export class DriveService {
	...
	private async getMetaData(fileUrl: string) {
					...
					content: [
						{
							...
							text:
								"添付されたファイルを分析してください。\n" +
								'・画像の場合はイラストか写真かを分類し、imageTypeプロパティにイラストの場合は "illust"写真の場合は "picture" と入力してください。\n' +
								"・特徴を示すラベルを日本語でlabelsプロパティに入力してください。\n" +
								"回答は以下のJSON形式で記載してください。\n" +
								"{\n" +
								'  "imageType":  "",\n' +
								'  "labels": [],\n' +
								"}\n",
	...
	private async save(
		...
		if (meta.useObjectStorage) {
			...
			if (
				FILE_TYPE_IMAGES.includes(type) &&
				this.config.openaiApiKey &&
				this.config.openaiApiKey !== ""
			) {
				file.metadata = await this.getMetaData(url);
			}

			return await this.driveFilesRepository
				.insert(file)
				.then((x) =>
					this.driveFilesRepository.findOneByOrFail(x.identifiers[0]),
				);
		} else {
	...
	public async addFile({
		...
		if (isLink) {
			try {
				...
				file = await this.driveFilesRepository
					.insert(file)
					.then((x) =>
						this.driveFilesRepository.findOneByOrFail(x.identifiers[0]),
					);
