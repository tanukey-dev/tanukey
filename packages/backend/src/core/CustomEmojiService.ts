...
import { Emoji, EmojiStatus } from "@/models/entities/Emoji.js";
import type { DriveFilesRepository } from "@/models/Repositories.js";
import type { EmojisRepository, Role } from "@/models/Repositories.js";
...
export class CustomEmojiService implements OnApplicationShutdown {
	...
	constructor(
		...
		this.localEmojisCache = new RedisSingleCache<Map<string, Emoji>>(
			this.redisClient,
			"localEmojis",
			{
				...
				fetcher: () =>
					this.emojisRepository
						.find({ where: { host: IsNull() } })
						.then(
							(emojis) => new Map(emojis.map((emoji) => [emoji.name, emoji])),
						),
				toRedisConverter: (value) => JSON.stringify(Array.from(value.values())),
				fromRedisConverter: (value) => {
					if (!Array.isArray(JSON.parse(value))) return undefined; // 古いバージョンの壊れたキャッシュが残っていることがある(そのうち消す)
					return new Map(
						JSON.parse(value).map((x: Serialized<Emoji>) => [
							x.name,
							{
								...x,
	...
	public async add(data: {
		...
		const sameNameEmoji = await this.emojisRepository.findOneBy({
			name: data.name,
			host: IsNull(),
		});
		if (sameNameEmoji != null) throw new Error("name already exists");

		const emoji = await this.emojisRepository
			.insert({
	...
	public async populateEmojis(
		...
		const emojis = await Promise.all(
			emojiNames.map((x) => this.populateEmoji(x, noteUserHost)),
		);
		const res = {} as any;
	...
	public async prefetchEmojis(
		...
		const notCachedEmojis = emojis.filter(
			(emoji) => this.cache.get(`${emoji.name} ${emoji.host}`) == null,
		);
		...
		const _emojis =
			emojisQuery.length > 0
				? await this.emojisRepository.find({
						where: emojisQuery,
